...
namespace Graphics
{
	...
	void CPostOverlay::Initialize()
	{
		...
		m_pPostMat = reinterpret_cast<CMaterial*>(Resources::CManager::Instance().GetResource(Resources::RESOURCE_TYPE_MATERIAL, Math::FNV1a_64("MATERIAL_POST_OVERLAY")));
		...
		const CRenderTexture** ppPostTextureList, u32 postTextureCount, u32& outRenderTextureCount)
	{
		...
		static const u32 sceneDepthHash = Math::FNV1a_32("sceneDepthTexture");

		m_pPostMat->SetTexture(colorHash, ppTargetTextureList[0]->GetRTVTexture(0));
		m_pPostMat->SetTexture(sceneDepthHash, m_data.pViewScene->GetPostProcessor().GetRenderTexture().GetDSVTexture());
